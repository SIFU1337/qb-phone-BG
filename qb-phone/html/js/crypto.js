...
function SetupCryptoData(Crypto) {
    ...
    $(".crypto-action-page-wallet").html("Портфейл: "+CryptoData.Portfolio+" Qbit(и)");
    ...
    if (CryptoData.History.length > 0) {
        ...
        $.each(CryptoData.History, function(i, change){
            ...
            var Element =   '<div class="cryptotab-course-block">' +
                                ...
                                '<span class="cryptotab-course-block-title">Промяна на стойността</span>' +
                                '<span class="cryptotab-course-block-happening"><span style="font-size: 1.3vh;">$'+change.PreviousWorth+'</span> до <span style="font-size: 1.3vh;">$'+change.NewWorth+'</span>'+PercentageElement+'</span>' +
                            '</div>';
...
$(document).on('click', '#buy-crypto', function(e){
    ...
    if ((Coins !== "") && (Price !== "")) {
        if (QB.Phone.Data.PlayerData.money.bank >= Price) {
            $.post('https://qb-phone/BuyCrypto', JSON.stringify({
                ...
                if (CryptoData !== false) {
                    ...
                    QB.Phone.Notifications.Add("fas fa-university", "QBank", "&#36; "+Price+",- бяха изтеглени от баланса ви!", "#badc58", 2500);
                } else {
                    QB.Phone.Notifications.Add("fas fa-chart-pie", "Крипто", "Нямате достатъчно пари..", "#badc58", 1500);
                }
            });
        } else {
            QB.Phone.Notifications.Add("fas fa-chart-pie", "Крипто", "Нямате достатъчно пари..", "#badc58", 1500);
        ...
        QB.Phone.Notifications.Add("fas fa-chart-pie", "Крипто", "Попълнете всички полета!", "#badc58", 1500);
    }
});
        ...
        if (CryptoData.Portfolio >= parseInt(Coins)) {
            $.post('https://qb-phone/SellCrypto', JSON.stringify({
                ...
                if (CryptoData !== false) {
                    ...
                    CloseCryptoPage()
                    QB.Phone.Data.PlayerData.money.bank = parseInt(QB.Phone.Data.PlayerData.money.bank) + parseInt(Price);
                    QB.Phone.Notifications.Add("fas fa-university", "QBank", "&#36; "+Price+",- бяха добавени към баланса ви!", "#badc58", 2500);
                } else {
                    QB.Phone.Notifications.Add("fas fa-chart-pie", "Крипто", "Нямате достатъчно Qbits..", "#badc58", 1500);
                }
            });
        } else {
            QB.Phone.Notifications.Add("fas fa-chart-pie", "Крипто", "Нямате достатъчно Qbits..", "#badc58", 1500);
            ...
            if (WalletId !== CryptoData.WalletId) {
                $.post('https://qb-phone/TransferCrypto', JSON.stringify({
                    ...
                    if (CryptoData == "notenough") {
                        ...
                        QB.Phone.Notifications.Add("fas fa-university", "Крипто", "Този Wallet-ID не съществува!", "#badc58", 2500);
                    } else {
                        UpdateCryptoData(CryptoData)
                        CloseCryptoPage()
                        QB.Phone.Notifications.Add("fas fa-university", "Крипто", "Прехвърлихте "+Coins+",- на "+WalletId+"!", "#badc58", 2500);
                    }
                });
            } else {
                QB.Phone.Notifications.Add("fas fa-university", "Крипто", "Не можете да прехвърлите на себе си..", "#badc58", 2500);
            }
        } else {
            QB.Phone.Notifications.Add("fas fa-chart-pie", "Крипто", "Нямате достатъчно Qbits..", "#badc58", 1500);
        }
...
$(".crypto-action-page-buy-crypto-input-coins").keyup(function(){
    ...
    $(".crypto-action-page-buy-crypto-input-money").html(MoneyAmount+" Долара");
});
...
$(".crypto-action-page-sell-crypto-input-coins").keyup(function(){
    ...
    $(".crypto-action-page-sell-crypto-input-money").html(MoneyAmount+" Долара");
});
